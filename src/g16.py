...
# plot_s.get_figure().savefig("./out/correlation.png", dpi=200)
...
fit = SelectKBest().fit(data[symptoms + ["TOTAL"]], data['class'])
plot_st = sb.barplot(x=fit.scores_, y=symptoms + ["TOTAL"], orient="h")
plot_st.get_figure().savefig("./out/kbest_features.png", dpi=200)
plot_st.get_figure().clf()
...
correlation = correlation.corr(method="spearman", min_periods=44000)
plot_s = sb.heatmap(correlation, annot=True)    
plot_s.get_figure().savefig("./out/correlation_2.png", dpi=200)
plot_s.get_figure().clf()
print(correlation)
print()
